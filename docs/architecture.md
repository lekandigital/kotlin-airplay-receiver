...
`VideoPlayer` is a dedicated thread around Android `MediaCodec`. The video path keeps a small queue, preserves codec config, keeps frame dependencies intact before decode, and drains decoder output so only the newest waiting output frame is rendered. The decoder discards a few startup outputs after codec setup, and the startup pane is removed only after four rendered video outputs after that, giving the display surface a short burst of clean frames before controls disappear. On the ThinkSmart View target it defaults to H.264 at 1280x720 and renders decoded frames directly to the centered 16:9 `SurfaceView`, leaving black bars on the 1280x800 panel instead of stretching the stream. The optional 1920x1080 mode uses the same decode path and relies on the display surface for downscaling without forcing an oversized fixed surface buffer.
